package com.linsh.lshapp.lib.qcloud.sign;


import com.linsh.lshapp.lib.qcloud.common_utils.CommonCodecUtils;
import com.linsh.lshapp.lib.qcloud.common_utils.CommonPathUtils;
...
public class Sign {
     ...
     * @param cosPath    要签名的cos路径
     * @param expired    超时时间
     * @param uploadFlag 除了生成下载签名，其他情况updateFlag皆为true
    ...
    private static String appSignatureBase(Credentials cred, String bucketName, String cosPath, long expired,
        ...
        String fileId = null;
        if (uploadFlag) {
            fileId = String.format("/%s/%s%s", appId, bucketName, cosPath);
        } else {
            fileId = cosPath;
        }
     ...
     * @param cosPath    要签名的cos路径
     * @param cred       用户的身份信息, 包括appid, secret_id和secret_key
     * @param expired    签名过期时间, UNIX时间戳。如想让签名在30秒后过期, 即可将expired设成当前时间加上30秒
     * @return base64编码的字符串
     */
    public static String getPeriodEffectiveSign(String bucketName, String cosPath, Credentials cred, long expired) throws Exception {
     ...
     * @return base64编码的字符串
     */
    public static String getOneEffectiveSign(String bucketName, String cosPath, Credentials cred) throws Exception {
     ...
     * @return base64编码的字符串
     */
    public static String getDownLoadSign(String bucketName, String cosPath, Credentials cred, long expired) throws Exception {
